...
The most proficient users of R for data analysis that I've encountered have tended to prefer data.table. However, there are many more online tutorials available for dplyr. That is why some emphasis on data.table is worthwhile here. 
...
For information on dplyr, see its webpage: https://dplyr.tidyverse.org/. The linked page mentions five key dplyr functions. The dplyr functions and their data.table equivalents:
...
* `arrange()` can be done with `setkey()` (see section \@ref(data-table-functions)), or a few other options.
