...
Most R data analysis involves working on data that is read from somewhere. This chapter covers ways of reading data, and where to store them.

## Common functions for importing data

Common functions for importing data include:
...
* <u>fread()</u> from the <u>data.table package</u>. Reads a csv file in as a data table, much faster than read.csv().
...
<b>To save data</b> from your R session to a local location in a common file format, use the functions above but replace `read` with `write`, for example, `write.csv()`.
...
RStudio projects are suitable for all coding tasks. The project creates a folder that contains the scripts and other files used for that task, such as locally stored datasets.
...
iris_dataset <- read.csv('iris_dataset', row.names=FALSE)
```

A key benefit of RStudio projects is that it keeps your task self contained. It will keep working if you move it to another folder (as long as you move the whole project folder) or if you give it to someone else.
